...
int ev_keypressed(int keycode, t_glob *g)
{
	if (keycode == KEY_ESC)
		ev_destroy(g);
	if (keycode == KEY_A)
		g->world.cam.pos = vec_sub(g->world.cam.pos, vec_mult(vec_cross(vec3(0, 1, 0), g->world.cam.normal), 0.1));
	if (keycode == KEY_D)
		g->world.cam.pos = vec_add(g->world.cam.pos, vec_mult(vec_cross(vec3(0, 1, 0), g->world.cam.normal), 0.1));
	if (keycode == KEY_W)
		g->world.cam.pos = vec_add(g->world.cam.pos, vec_mult(g->world.cam.normal, 0.1));
	if (keycode == KEY_S)
		g->world.cam.pos = vec_sub(g->world.cam.pos, vec_mult(g->world.cam.normal, 0.1));
	if (keycode == KEY_UP)
