...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/10 17:04:08 by tjukmong          #+#    #+#             */
/*   Updated: 2024/01/10 17:04:32 by tjukmong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	l_point_light(t_world *w, t_hittable *rec, t_object light, t_ray r)
{
	...
	brightness = 0;
	if (theta < HALF_PI && (light_rec.t == -1
			|| vec_mag(vec_sub(light.pos, light_ray.origin)) < light_rec.t))
		brightness = fixed_to_double(light.brightness)
			* (1.0 - (theta / HALF_PI));
	rec->intensity -= 1 - brightness;
	set_color(&rec->color,
		color_sub(rec->color, color_mult_norm(
				color_invrt(light.abs_color), brightness)));
}

// No color
//set_color(&rec->color, color_mult_norm(light.abs_color, brightness));
// Inverted color
// set_color(&rec->color, color_sub(
//	rec->color, color_mult_norm(light.abs_color, brightness)));
